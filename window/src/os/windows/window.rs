    ...
    fn get_clipboard_data(&self, _clipboard: Clipboard) -> Future<ClipboardData> {
        Future::result(
            clipboard_win::get_clipboard_string()
                ...
                .context("Error getting clipboard data"),
        )
    }
        ...
        Some(cursor) => unsafe {
            SetCursor(LoadCursorW(
                ...
                match cursor {
                    ...
                    // Win32 has no closed-hand system cursor. The hand cursor
                    // still makes an active tab drag visually distinguishable.
                    MouseCursor::Grabbing => IDC_HAND,
                },
            ));
        },
    }
